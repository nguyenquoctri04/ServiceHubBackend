...
## Sử dụng CodeGraph (Using CodeGraph)
- **LUÔN LUÔN DÙNG CODEGRAPH**: Trước khi sử dụng grep/find hoặc đọc thủ công các file để tìm hiểu luồng code hoặc định vị các Symbol/Class/Function/File, Agent bắt buộc phải sử dụng công cụ CodeGraph (MCP tool `codegraph_explore` hoặc lệnh CLI `codegraph explore`) để truy vấn. Dự án đã được index sẵn CodeGraph với thư mục `.codegraph` tại thư mục gốc của repository.
